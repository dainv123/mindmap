...
**Q: Khi nào nên dùng Hooks và khi nào dùng Class Components?**

**A:**
```javascript
// ✅ Use Hooks when:
const hooksUseCases = {
    newCode: 'Writing new React components',
    functional: 'Prefer functional programming',
    simplicity: 'Want simpler component logic',
    performance: 'Better tree-shaking and optimization'
};

// ✅ Use Class Components when:
const classUseCases = {
    legacy: 'Maintaining existing class components',
    errorBoundaries: 'Need error boundary functionality',
    lifecycle: 'Complex lifecycle management',
    team: 'Team prefers class components'
};

// Hooks advantages
const hooksAdvantages = [
    'No need to bind methods',
    'Easier to test',
...
**Q: Cách optimize performance trong React?**

**A:**
```javascript
const performanceOptimizations = {
    // Prevent unnecessary re-renders
    reactMemo: 'Use React.memo for expensive components',
    useCallback: 'Memoize functions with useCallback',
    useMemo: 'Memoize expensive calculations with useMemo',
    
    // Code splitting
    lazyLoading: 'Use React.lazy for route-based splitting',
    dynamicImports: 'Use dynamic imports for conditional loading',
    
    // Bundle optimization
    treeShaking: 'Use ES6 modules for better tree shaking',
    ...
    context: 'Use Context API sparingly',
    redux: 'Use Redux only when needed'
};
```

### 3. State Management
**Q: Khi nào nên dùng Context API vs Redux vs local state?**
...
const stateManagement = {
    ...
    localState: {
        ...
        example: 'Form inputs, UI state',
        pros: 'Simple, no setup required',
        cons: 'Not shareable between components'
    },
    
    // Context API
    context: {
        when: 'Shared data across component tree',
        example: 'Theme, user authentication',
        pros: 'Built into React, simple setup',
        cons: 'Can cause unnecessary re-renders'
    },
    
    // Redux
    redux: {
        when: 'Complex state, multiple sources',
        example: 'E-commerce cart, user management',
        pros: 'Predictable state, dev tools',
    ...
    reactQuery: {
        ...
        pros: 'Automatic caching, background updates',
...
**Q: Chiến lược testing cho React applications?**
...
### 5. Custom Hooks
**Q: Làm sao tạo custom hooks hiệu quả?**

**A:**
```javascript
const customHooksBestPractices = {
    // Naming conventions
    naming: 'Always start with "use" prefix',
    examples: ['useCounter', 'useApi', 'useLocalStorage'],
    
    // Single responsibility
    responsibility: 'One hook should do one thing well',
    example: 'useApi for data fetching, useForm for form handling',
    
    // Reusability
    reusability: 'Make hooks generic and reusable',
    example: 'useApi(url) instead of useUsers()',
    
    // Error handling
    errorHandling: 'Always handle errors and loading states',
    example: 'Return { data, loading, error }',
    
    // Dependencies
    dependencies: 'Be careful with useEffect dependencies',
    example: 'Use useCallback for function dependencies',
    
    // Testing
    testing: 'Test custom hooks with renderHook',
    example: 'Test all possible states and scenarios'
};

// Example of good custom hook
